...
export default class DeviceRoutes implements IRoutes {
  ...
  //prettier-ignore
  registerRoutes(): void {
    this.router.get("/", this.auth.handle(), this.controller.userAll);
    this.router.put("/", this.auth.handle(), this.controller.userStore);
    this.router.patch("/:id", this.auth.handle(), this.controller.userUpdate);
